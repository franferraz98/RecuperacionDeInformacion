...
public class InferenciaOWL2tesauro{
	...
	public static void main (String args[]) throws Exception{
		...
		String queryString = "PREFIX dc: <http://purl.org/dc/terms/>"
						   + "PREFIX gemet: <http://www.gemet.com/>"
						    + "SELECT ?libro WHERE {"
							 	   + "?libro dc:subject gemet:biosfera }" ;
		
		//ejecutamos la consulta y obtenemos los resultados
		Query query = QueryFactory.create(queryString) ;
		  QueryExecution qexec = QueryExecutionFactory.create(query, colec) ;
		  try {
		    ResultSet results = qexec.execSelect() ;
		    for ( ; results.hasNext() ; )
		    {
		      QuerySolution soln = results.nextSolution() ;
		      Resource x = soln.getResource("libro");
		  ...
		  System.out.println("--------------------EJERCICIO 2--------------------");
			...
			//definimos la consulta (tipo query)
			String queryString2 = "PREFIX dc: <http://purl.org/dc/terms/>"
							   + "PREFIX gemet: <http://www.gemet.com/>"
							    + "SELECT ?libro WHERE {"
								 	   + "?libro dc:subject gemet:biosfera }" ;
